...
Traditional RBAC roles are usually broad groupings with no context awareness. They are static and they are defined by Identity Provider(IDP), not by Cerbos. Cerbos provides Derived roles are a way of augmenting those broad roles with contextual data to provide more fine-grained control at runtime. For example, a person with the broad static role `manager` can be augmented to `manager_of_scranton_branch` by taking into account the geographic location (or another factor) and giving that derived role bearer extra privileges on resources that belong to the Scranton branch.
